...
/**
 * Returns the absolute path to the viewcount file for the given basename.
 * Throws an error if the file does not exist.
 * @param {string} basename
 * @returns {Promise<string>}
 */
export async function getViewCountPath(basename) {
  const viewCountPath = absolute(
    `<gitRoot>/data/computed/${basename}/viewcount.json`,
  );
  ...
  return viewCountPath;
}

/**
 * Returns the absolute path to the heatmap file for the given basename.
 * Throws an error if the file does not exist.
 * @param {string} basename
 * @returns {Promise<string>}
 */
export async function getHeatmapPath(basename) {
  const heatmapPath = absolute(
    `<gitRoot>/data/computed/${basename}/heatmap.json`,
  );
